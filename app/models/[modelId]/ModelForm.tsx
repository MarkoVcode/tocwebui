  ...
  return (
    <form className="space-y-4">
      ...
      {/* {parameters.map((param, index) => (
        <div key={param.label} className="space-y-2"> onChange={onChange}
          <Label htmlFor={param.label}>{param.label}</Label>
          <Input
            id={param.label}
            type="range"
            min={param.min}
            max={param.max}
            value={param.value}
            onChange={(e) => onParameterChange(index, Number(e.target.value))}
          />
